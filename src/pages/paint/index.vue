  ...
  <div class="container">
    <scroll-view scroll-x class="index-list">
      <div class="index-list-box">
        <div class="index-list-item" v-for="(item, index) in paintList" :key="index">
          <img
            :src="prefix+item.pic_icon_active"
            @load="setWidth(index,$event)"
            data-index="index"
            v-bind:style="{ width: index==2?'185rpx':'145rpx'}"
            class="index-list-item-img"
            @click="changeArticle(index,item)"
            v-if="activeIndex == index"
          >
          <img
            :src="prefix+item.pic_icon"
            @load="setWidth(index,$event)"
            data-index="index"
            v-bind:style="{ width: index==2?'185rpx':'145rpx'}"
            class="index-list-item-img"
    ...
    <movable-area class="m-area">
      <movable-view
        ...
        @change="startTouch"
      >
        <img :src="mainPic" @load="imageLoad" lazy-load :style="{ width: imgWidth + 'rpx'}">
      </movable-view>
    </movable-area>
  </div>
</template>
